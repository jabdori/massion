...
import { initializeMassionBackend } from './backend/massionBoot';
...
app.whenReady().then(async () => {
    ...
    // Boot Massion backend — reads CLI config, resolves token, registers MassionSessionStore
    markStart('database-init');
    try {
        runtimeSessionStore = await initializeMassionBackend();
        markEnd('database-init');
        logger.main.info('Massion backend initialized');
    } catch (error) {
        logger.main.error('Massion backend init failed:', error);
        dialog.showErrorBox(
            'Massion - Backend Initialization Failed',
            `Failed to connect to Massion backend.\n\nError: ${error instanceof Error ? error.message : String(error)}\n\nEnsure "massion init" has been run first.`
        );
        app.quit();
        return;
    }
    ...
    // AI service disabled — Massion server handles agent execution
    ...
    // Check for pending restart continuations and queue continuation prompts
    if (aiService) await checkForRestartContinuation(aiService);
