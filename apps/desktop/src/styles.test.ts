...
describe("스타일 의미 토큰", () => {
   ...
   */
  /*
   * 사각 셋을 알고 쓰십시오. (1) translate(group.title)처럼 동적 인자는 리터럴이 아니라 안 보입니다.
   * (2) translate()를 아예 거치지 않는 문자열은 이 검사 밖입니다. (3) room.tsx는 미등록 20건이
   * 남아 있어 아직 대상이 아닙니다 — 첫날부터 실패하는 가드는 꺼지므로, 그 부채를 갚을 때 함께 넣습니다.
   */
