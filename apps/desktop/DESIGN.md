...
**수신함 항목은 예약색으로 종류를 가릅니다.** 승인 대기 = `◇` + `gate`(노랑, "사람 결정 필요") + `[거절][승인]`. 차단 = `⊘` + `halt`(빨강, "실행 막힘"). 연결 업무가 있는 항목은 상단 행을 `제목 — 상태 — 꺾쇠` 순서로 배치하고 맨 오른쪽 꺾쇠로 이동을 알립니다. 두 예약색을 뜻대로 쓰므로 색만 봐도 "내가 결정할 것"과 "내가 풀어줄 것"이 갈립니다. 수신함은 처리하는 곳이라 본문엔 사람의 말만 두고, 감사 식별자는 «식별자는 사람의 말 뒤에 선다»보다 한 단계 더 내려 `title` 툴팁으로만 둡니다.
