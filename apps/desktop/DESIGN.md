...
승인 게이트는 협업방·알림·대표·홈 어디서 보든 같은 문법입니다: `◇` 기호, `gate-wash` 배경, 무엇이·왜·되돌리기를 버튼보다 먼저. 같은 모양이 화면마다 다르게 동작하면 사용자가 같은 것을 두 번 배워야 합니다. 알림 패널을 닫는 것은 읽음 처리나 해결이 아니며, 도메인 결정 전까지 항목과 전역 배지가 유지됩니다.
...
**다른 곳으로 보내는 버튼은 `OpenButton` 하나뿐입니다.** 카드 전체를 버튼으로 두지 않으므로 유일한 진입점이고, 그래서 면을 가져야 합니다(`bg-raised` + `border-control`). 테두리만 있으면 카드 테두리에 섞여 보이지 않습니다.
