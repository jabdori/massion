...
**표면 일곱(홈·업무·조직·지식·개선·확장·설정)은 예외 없이 같은 3열 치수를 쓴다.** 단 `지식`은 노드를 골랐을 때만 시트가 열려 셋이 되고, 고르지 않았으면 캔버스가 폭을 다 쓴다. 표면마다 다르면 표면을 옮길 때마다 눈이 다시 자리를 찾아야 한다.

한 번 어긴 적이 있다. 조직 지도를 "판독 폭 확보"를 이유로 45%까지 넓혀 2열(645/645)로 갔는데, 노드가 라벨 없는 9px 점이라 넓힌 만큼 읽히는 것이 늘지 않았다(잉크 점유율 0.65%, 콘텐츠가 캔버스보다 넓어 우측이 잘림). 골격과 배경 열만 잃었다. **골격을 깨는 대가를 치르려면 그만큼 읽히는 것이 늘어야 한다.** 늘지 않으면 규칙이 옳다.
...
- **예외: 수신함.** 수신함은 읽는 흐름이 아니라 대기 큐이고 근거를 소유하지 않습니다. 근거 없이 누르는 승인 버튼을 만들지 않기 위해 결정을 소유 화면으로 보냅니다(아래 «같은 것은 같게 보입니다»).
