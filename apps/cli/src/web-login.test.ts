...
describe("Web Console 진입", () => {
  it("인증된 profile로 로컬 세션을 발급하고 code 노출 없이 Web root를 연다", async () => {
    ...
    const result = await openWebConsole({
      ...
      fetcher: async (url, init) => {
        ...
        return new Response(JSON.stringify({ schemaVersion: "massion.web.local-session.v1" }), {
          status: 201,
    ...
    const expectedUrl = "http://127.0.0.1:7331/";
    expect(result).toMatchObject({ url: expectedUrl, code: "" });
    expect(Date.parse(result.expiresAt)).toBeGreaterThanOrEqual(before + 28_800_000);
    expect(opened).toBe(expectedUrl);
    expect(calls).toHaveLength(1);
    expect(calls[0]?.url).toBe("http://127.0.0.1:7331/api/v1/web/local-session");
    expect(calls[0]?.init?.headers).toMatchObject({
      authorization: "Bearer owner-token",
      origin: "http://127.0.0.1:7331",
      "sec-fetch-site": "same-origin",
    });
    expect(String(calls[0]?.init?.body)).toContain('"commandId"');
  });
  ...
  it("ticket response가 유효하지 않으면 ticket 발급을 거부한다", async () => {
    let opened = false;
    await expect(
      issueWebLoginTicket({
  ...
  it("로컬 세션 발급이 거부되면 Web을 열지 않는다", async () => {
    let opened = false;
    await expect(
      openWebConsole({
        endpoint: "https://massion.example.com",
        token: "owner-token",
        fetcher: async () => new Response(JSON.stringify({ detail: "denied" }), { status: 401 }),
        openBrowser: async () => {
          opened = true;
        },
      }),
    ).rejects.toThrow("Local session 발급");
    expect(opened).toBe(false);
  });
});
