...
function manifest(value: unknown): readonly BundledExtension[] {
  ...
  const entries = value.map((item) => {
    if (!item || typeof item !== "object" || Array.isArray(item))
      throw new Error("공식 Extension 항목이 유효하지 않습니다");
...
export async function seedBundledOfficialExtensions(input: {
  ...
  const registration = await input.identities.registerPersonalUser({
    email: OFFICIAL_EMAIL,
    displayName: "Massion Extensions",
  });
  const context = await input.organizations.resolveTenantContext(
  ...
  for (const entry of entries) {
    ...
    if (digest !== entry.artifactDigest)
      throw new Error(`공식 Extension artifact digest가 일치하지 않습니다: ${entry.packageName}`);
    ...
    const staged = await input.versions.stage(
      context,
      `official-seed-${entry.packageName.slice("@massion-ext/".length)}-${entry.packageVersion}`,
      {
