...
export interface AssembleBuiltinModelRoutesInput {
  ...
  readonly observed: ObservedMiniMaxSubscriptionModel;
}

export interface AssembledBuiltinModelRoutes {
  readonly modelId: "MiniMax-M2.7" | CodexGpt56ModelId;
...
export class BuiltinModelRouteAssembler {
  ...
  public async assemble(
    ...
    if (input.providerId !== "minimax-token-plan") {
      throw new Error("현재 내장 Core model route 조립은 MiniMax Token Plan만 지원합니다");
    }
    if (input.observed.modelId !== MINIMAX_MODEL_ID || !input.observed.availableModelIds.includes(MINIMAX_MODEL_ID)) {
    ...
    const verificationEvidence = miniMaxEvidence(input.accountId, input.observed);
    if (!profile) {
      profile = (
      ...
      if (!currentMiniMaxEvidence(existingEvidence, input.accountId, input.observed)) {
