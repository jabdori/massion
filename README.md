...
이미 연결된 profile이 있으면 다시 로그인하지 않고 재사용합니다. 로그인 또는 추가 계정 연결이 필요한 경우에만 공급자 인증 화면이 나타납니다. 모델을 연결하지 않아도 TUI·조직·업무 기록·백업 기능은 확인할 수 있습니다.

### 4. 첫 업무 실행

연결된 모델이 있으면 자연어로 업무를 전달합니다.

```sh
massion run "이 저장소의 README를 초보자용으로 요약해줘" --wait
```

결과는 다음 세 상태 중 하나로 끝납니다.
...
완료된 업무와 작업은 TUI의 `3` 업무 화면 또는 다음 명령으로 다시 볼 수 있습니다.
...
같은 개인 조직을 브라우저에서 보려면 TUI를 닫고 Web Console을 엽니다.
