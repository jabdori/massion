...
`massion init`을 인자 없이 실행하면 소유자 이메일과 표시 이름을 묻는 온보딩이 시작됩니다. 인자 없이 처음 `massion`을 실행해도 설정 파일이 없으면 같은 온보딩을 마친 뒤 곧바로 TUI를 엽니다. 개인용 기본 서버(`http://127.0.0.1:7331`)도 자동으로 준비하므로 일반적인 첫 실행에서는 `massion local start`를 따로 입력할 필요가 없습니다. 기존 스크립트와 자동화 환경에서는 `massion init http://127.0.0.1:7331 owner@example.com "내 이름"` 형식도 계속 사용할 수 있습니다.
