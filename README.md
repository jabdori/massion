...
`massion init`을 인자 없이 실행하면 소유자 이메일과 표시 이름을 묻는 온보딩이 시작됩니다. 인자 없이 처음 `massion`을 실행해도 설정 파일이 없으면 같은 온보딩으로 안내되므로, 초기화 전에 TUI가 조용히 종료되지 않습니다. 개인용 기본 서버(`http://127.0.0.1:7331`)도 자동으로 준비하므로 일반적인 첫 실행에서는 `massion local start`를 따로 입력할 필요가 없습니다. 기존 스크립트와 자동화 환경에서는 `massion init http://127.0.0.1:7331 owner@example.com "내 이름"` 형식도 계속 사용할 수 있습니다.

초기화가 끝난 뒤 인자 없이 `massion`을 실행하면 터미널 사용자 인터페이스(TUI)가 열리고, 필요한 개인용 서버를 먼저 확인·시작합니다. 서버를 시작할 수 없는 경우에는 오류를 숨기지 않고 터미널에 원인을 출력합니다. `massion --web`도 같은 방식으로 개인용 서버를 자동 확인한 뒤, 인증된 로컬 서버에 5분 유효한 일회성 로그인 티켓을 만들고 기본 브라우저를 엽니다.
...
개인용 profile에서는 별도의 서버 시작 명령 없이 Web Console을 엽니다.
...
다음 명령으로 실제 실행 파일과 터미널 조건을 확인합니다.
...
`command -v`가 비어 있으면 설치 경로를 PATH에 추가합니다.

```sh
export PATH="$HOME/.local/bin:$PATH"
```

TUI는 실제 대화형 터미널(standard input/output가 TTY)에서만 열리고 최소 `80×24` 크기가 필요합니다. SSH 명령의 출력 리다이렉션, CI, 파이프 안에서는 TUI 대신 CLI 모드를 사용해주세요.
