...
이 directory에는 실제 secret을 commit하지 않습니다. 다음 여섯 파일을 owner-only 권한으로 준비합니다.
...
다른 경로를 쓰려면 `MASSION_DATABASE_OWNER_PASSWORD_FILE`, `MASSION_DATABASE_PASSWORD_FILE`, `MASSION_TOKEN_KEY_FILE`, `MASSION_REGISTRY_KEY_FILE`, `MASSION_TLS_CERTIFICATE_FILE`, `MASSION_TLS_PRIVATE_KEY_FILE`을 설정합니다.
