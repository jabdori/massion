...
export type ClaimApplicationRunResult =
  | { readonly outcome: "claimed"; readonly leaseGeneration: number; readonly recovered: boolean }
...
export class ApplicationRunStore {
  ...
  public static async create(
    ...
    await applyMigrations(database, [APPLICATION_RUN_MIGRATION]);
  ...
  public async claim(
    context: TenantContext,
    runId: string,
    options: { readonly resumeAwaitingApproval?: boolean; readonly resumeBlocked?: boolean } = {},
    ...
    return await this.database.transaction(async (transaction) => {
      ...
      await transaction.query(
        "UPDATE application_run SET status = 'running', lease_generation = $generation, lease_expires_at = <datetime>$expires_at, approval_id = NONE, blocked_reason = NONE, updated_at = <datetime>$updated_at WHERE organization_id = $organization_id AND run_id = $run_id AND status = $previous_status AND lease_generation = $previous_generation;",
        {
          organization_id: context.organizationId,
          run_id: runId,
      ...
      return { outcome: "claimed", leaseGeneration: generation, recovered };
    });
  }
  ...
  public async advance(
    ...
    return await this.transition(context, runId, generation, async (transaction, record) => {
      await transaction.query(
        "UPDATE application_run SET status = 'ready', stage = $stage, work_id = $work_id, lease_expires_at = NONE, updated_at = <datetime>$updated_at WHERE organization_id = $organization_id AND run_id = $run_id AND status = 'running' AND lease_generation = $generation;",
        {
          organization_id: context.organizationId,
          run_id: runId,
          generation,
