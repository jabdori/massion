...
export class SigstoreProvenanceVerifier {
  ...
  public constructor(options: { readonly cryptographicVerify?: CryptographicVerify } = {}) {
    this.cryptographicVerify = options.cryptographicVerify ?? (async (bundle) => await verifySigstore(bundle, { timeout: 5_000 }));
  }
  ...
  public async verify(artifact: Buffer, bundleValue: unknown, policy: ProvenancePolicy): Promise<ProvenanceResult> {
    ...
    if (payload.length === 0 || payload.length > 256 * 1024) throw new Error("provenance payload byte 상한을 초과했습니다");
    ...
    if (statement._type !== "https://in-toto.io/Statement/v1") throw new Error("in-toto statement type이 유효하지 않습니다");
    if (typeof statement.predicateType !== "string" || !statement.predicateType.startsWith("https://slsa.dev/provenance/"))
    ...
    if (!identity || !policy.identity.test(identity)) throw new Error("provenance certificate identity가 trust policy와 다릅니다");
    const predicate = record(statement.predicate, "provenance predicate");
    const definition = predicate.buildDefinition;
    const parameters = definition && typeof definition === "object" ? (definition as Record<string, unknown>).externalParameters : undefined;
    const sourceRepository = parameters && typeof parameters === "object" ? (parameters as Record<string, unknown>).source : undefined;
    return {
      outcome: "pass",
      issuer,
      identity,
