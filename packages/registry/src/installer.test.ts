...
describe("Registry installer", () => {
  it("grant·recall·artifact digest·archive identity를 재검증한 뒤 Host에 verified install한다", async () => {
    ...
    const installRegistry = vi.fn(async () => ({
    ...
    const installer = new RegistryInstaller({
      ...
      inspectArchive: vi.fn(
        async () =>
          ({
            manifest: { name: version.packageName, version: version.packageVersion },
            artifactDigest: digest,
          }) as never,
      ),
    ...
    expect(installRegistry).toHaveBeenCalledWith(
      context,
      expect.objectContaining({ archive: body, trustLevel: "verified" }),
    );
  });
  ...
  it("download 뒤 byte가 바뀌면 Host를 호출하지 않는다", async () => {
    ...
    await expect(
      installer.install(context, {
        commandId: "registry-install-0002",
        downloadGrant: "grant",
        environment: "production",
        riskClass: "medium",
