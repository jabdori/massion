...
function compatible(version: RegistryVersion, runtime: { readonly agentOS: string; readonly node: string; readonly surrealDB?: string }): boolean {
  ...
  for (const [key, actual] of [["agentOS", runtime.agentOS], ["node", runtime.node], ["surrealDB", runtime.surrealDB]] as const) {
    const range = ranges[key];
    if (actual === undefined && key === "surrealDB") continue;
    if (typeof range !== "string" || actual === undefined || !semver.validRange(range) || !semver.satisfies(actual, range)) return false;
  }
  return true;
}
...
export class RegistryCatalog {
  ...
  public constructor(private readonly store: RegistryCatalogStore, options: { readonly tokenSecret: Buffer }) {
  ...
  public async search(input: {
    ...
    const candidates = (await this.store.list())
      .filter((version) => version.state === "published" && visible(version, input.organizationId) && compatible(version, input.runtime))
      .filter((version) => {
        const description = manifestRecord(version).description;
        return !query || version.packageName.includes(query) || (typeof description === "string" && description.toLocaleLowerCase("en-US").includes(query));
      })
      .sort((left, right) => left.packageName.localeCompare(right.packageName) || semver.rcompare(left.packageVersion, right.packageVersion));
  ...
  public async issueDownload(input: { readonly organizationId: string; readonly versionId: string }): Promise<{
    ...
    const payload = Buffer.from(JSON.stringify({ v: 1, versionId: version.versionId, digest: version.artifactDigest, organizationId: input.organizationId, expiresAt })).toString("base64url");
    return { token: `${payload}.${this.sign(payload)}`, artifactDigest: version.artifactDigest, expiresAt: new Date(expiresAt).toISOString() };
  }

  public async verifyDownload(token: string, organizationId: string): Promise<RegistryVersion> {
    const [payload, signature, extra] = token.split(".");
    if (!payload || !signature || extra || this.sign(payload) !== signature) throw new Error("download grant signature가 유효하지 않습니다");
    const value = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as { versionId?: unknown; digest?: unknown; organizationId?: unknown; expiresAt?: unknown };
    if (value.organizationId !== organizationId || typeof value.versionId !== "string" || typeof value.expiresAt !== "number" || value.expiresAt < Date.now())
  ...
  public async packument(organizationId: string, packageName: string): Promise<{
    readonly name: string;
    readonly "dist-tags": Record<string, string>;
    readonly versions: Record<string, { readonly name: string; readonly version: string; readonly description?: string; readonly deprecated?: string; readonly dist: { readonly tarball: string; readonly integrity: string } }>;
  }> {
    normalizePackageIdentity(packageName, "1.0.0");
    const candidates = (await this.store.list()).filter((version) => version.packageName === packageName && visible(version, organizationId) && version.state !== "staged").sort((left, right) => semver.rcompare(left.packageVersion, right.packageVersion));
    if (candidates.length === 0) throw new Error("Registry package를 찾을 수 없습니다");
    const versions: Record<string, { name: string; version: string; description?: string; deprecated?: string; dist: { tarball: string; integrity: string } }> = {};
    for (const version of candidates) {
      ...
      versions[version.packageVersion] = {
        ...
        ...(version.state === "recalled" ? { deprecated: `Massion recalled: ${recalls.at(-1)?.reason ?? "security policy"}` } : {}),
        dist: {
  ...
  private encodeCursor(offset: number, input: { readonly organizationId: string; readonly query: string; readonly runtime: unknown }): string {
    const payload = Buffer.from(JSON.stringify({ offset, organizationId: input.organizationId, query: input.query, runtime: input.runtime })).toString("base64url");
    return `${payload}.${this.sign(payload)}`;
  }

  private decodeCursor(cursor: string, input: { readonly organizationId: string; readonly query: string; readonly runtime: unknown }): number {
    const [payload, signature, extra] = cursor.split(".");
    if (!payload || !signature || extra || this.sign(payload) !== signature) throw new Error("Catalog cursor가 유효하지 않습니다");
    const value = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as { offset?: unknown; organizationId?: unknown; query?: unknown; runtime?: unknown };
    if (value.organizationId !== input.organizationId || value.query !== input.query || JSON.stringify(value.runtime) !== JSON.stringify(input.runtime) || !Number.isSafeInteger(value.offset) || (value.offset as number) < 0)
