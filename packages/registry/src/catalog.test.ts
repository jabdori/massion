...
async function published(
  ...
  input: { name: string; version: string; visibility?: "public" | "private"; owner?: string },
) {
...
describe("Registry catalog", () => {
  it("호환되는 public package만 결정론적으로 검색하고 private tenant를 격리한다", async () => {
    ...
    const publicResult = await catalog.search({
    ...
    const privateResult = await catalog.search({
  ...
  it("recalled version은 검색·download grant에서 차단하고 packument에 경고한다", async () => {
    ...
    await store.recall(version.versionId, {
      recallId: "recall-slack-1",
      category: "security",
      severity: "critical",
      reason: "GHSA-test",
    });
    ...
    await expect(catalog.issueDownload({ organizationId: "org-owner", versionId: version.versionId })).rejects.toThrow(
      "recalled",
    );
