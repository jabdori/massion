...
export type EvidenceValidationReason = "invalid" | "integrity" | "stale";

export class EvidenceValidationError extends Error {
  public constructor(
    public readonly reason: EvidenceValidationReason,
    message: string,
  ) {
    super(message);
...
function evidenceError(reason: EvidenceValidationReason, message: string): never {
...
function assertHash(value: string, label: string): void {
  if (!/^[a-f0-9]{64}$/u.test(value)) evidenceError("integrity", `${label}는 SHA-256 형식이어야 합니다`);
}

function timestamp(value: string, label: string): number {
  const parsed = new Date(value).getTime();
  if (!Number.isFinite(parsed)) evidenceError("invalid", `${label} 시각이 유효하지 않습니다`);
  return parsed;
}
...
function verifyFreshness(createdAt: string, observedAt: string, maximumAgeMs: number): void {
  if (!Number.isSafeInteger(maximumAgeMs) || maximumAgeMs < 0)
    evidenceError("invalid", "Evidence freshness 상한이 유효하지 않습니다");
  const created = timestamp(createdAt, "Evidence createdAt");
  const observed = timestamp(observedAt, "Evidence observedAt");
  if (created > observed) evidenceError("stale", "Evidence가 관측 시각보다 미래에 생성됐습니다");
  if (observed - created > maximumAgeMs) evidenceError("stale", "Evidence freshness 상한을 초과했습니다");
}
...
export function verifyArtifactEvidence(input: {
  ...
  if (artifact.organizationId !== input.organizationId)
    evidenceError("invalid", "ArtifactVersion organization이 일치하지 않습니다");
  if (artifact.workId !== input.workId) evidenceError("invalid", "ArtifactVersion Work가 일치하지 않습니다");
  if (!input.allowedArtifactVersionIds.includes(artifact.artifactVersionId)) {
    evidenceError("invalid", "허용된 ArtifactVersion ID가 아닙니다");
  }
  assertHash(artifact.checksum, "ArtifactVersion checksum");
  if (sha256(artifact.contentJson) !== artifact.checksum)
    evidenceError("integrity", "ArtifactVersion content checksum이 일치하지 않습니다");
...
export function verifyEvidenceBriefFreshness(input: {
  ...
  if (brief.organizationId !== input.organizationId)
    evidenceError("invalid", "EvidenceBrief organization이 일치하지 않습니다");
  if (brief.workId !== input.workId) evidenceError("invalid", "EvidenceBrief Work가 일치하지 않습니다");
  if (brief.status !== "ready") evidenceError("stale", "ready EvidenceBrief만 검증 증거로 사용할 수 있습니다");
  if (brief.repositoryRevisionId !== current.repositoryRevisionId) {
    evidenceError("stale", "EvidenceBrief repository revision이 현재 revision과 다릅니다");
  }
  if (brief.indexVersionId !== current.indexVersionId) {
    evidenceError("stale", "EvidenceBrief index version이 현재 index와 다릅니다");
  }
  if (brief.configurationChecksum !== current.configurationChecksum) {
    evidenceError("stale", "EvidenceBrief configuration checksum이 현재 구성과 다릅니다");
  }
  ...
  try {
    ...
    evidenceError("integrity", "EvidenceBrief references 또는 claims JSON이 올바르지 않습니다");
  }
  if (!Array.isArray(references) || !Array.isArray(claims)) {
    evidenceError("integrity", "EvidenceBrief references와 claims는 배열이어야 합니다");
  }
  ...
  if (checksum !== brief.checksum) evidenceError("integrity", "EvidenceBrief content checksum이 일치하지 않습니다");
