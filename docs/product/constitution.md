...
주요 의미 표면은 홈·업무·조직·개선·확장·설정 여섯 개입니다. 2026-07-24 사용자 논의에 따라 독립 `확인 필요` 표면은 제거하고 전역 `알림` 유틸리티로 통합했습니다.

- `알림`의 미해결 승인 항목은 단순 통보가 아니라 실행이 **멈춰서 사람을 기다리는 상태**입니다. 읽거나 패널을 닫아도 사라지지 않고 승인·거절·취소·만료 뒤에만 제거합니다. 배지는 읽지 않은 수가 아니라 조직 전체의 미해결 조치 수를 셉니다.
- `개선`은 자가개선의 **근거와 효과**를 소유합니다. 채택할 것이 생겼다는 사실은 `알림`에 뜨지만, 왜 바꾸자는지·반대 근거는 무엇인지·바꾸고 나서 나아졌는지는 이 표면이 소유합니다. 근거를 알림으로 축소하면 4.8의 "근거 기반의 보수적 채택"이 성립하지 않습니다.
- `알림`은 제품 표면이 아니라 어느 표면에서나 여는 전역 유틸리티입니다. 범용 알림 도메인이 생기기 전까지 실제 대기 승인만 투영합니다.
...
2026-07-24 기준. 표면 여섯(홈, 업무, 조직, 개선, 확장, 설정)과 전역 알림 패널이 구현돼 있습니다. 도메인 표면은 같은 3열 골격을 쓰고 알림은 현재 표면 위에 열립니다.
...
추가 화면 코드를 작성하기 전에 홈, 업무, 조직, 개선, 확장, 설정과 전역 알림의 역할과 이동 관계를 목업과 구현 설계로 고정합니다. 기존 Work 관제 화면은 업무 상세로 유지하고 AgentOS 전체 셸로 확장 해석하지 않습니다.

완료 조건:

- 여섯 표면의 사용자 목적과 핵심 행동이 중복되지 않고 알림은 표면을 대체하지 않습니다.
