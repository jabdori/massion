...
| 표면 | 사용자 목적 |
|---|---|
| 대표·홈 | 조직과 대화하고 새 사명을 맡기며 현재 중요한 상황을 파악 |
| 업무 | Work의 대화·계획·실행·산출물·검증·후속 지시를 관제 |
| 조직 | 부서·팀·전문가·책임·역량·현재 배치와 조직 변경 제안을 운영 |
| 결정 | 위험 실행, 조직 변경, 권한과 자기수정 결정을 통합 검토 |
| 성장 | Reflection, 개선 근거, 평가, 채택·거부·효과·되돌리기를 관리 |
| 역량 | Skill·Tool·MCP·Extension이 추가하는 실제 능력을 검색·설치·관리 |
...
- 독립 Tauri·React 셸과 Work 관제 화면은 구현 중입니다.
- 전역 메뉴는 업무, 확인 필요, 조직, 자동화, 확장으로 구성되어 있습니다.
- Representative·홈과 Growth 표면은 없습니다.
- 조직 화면은 현재 조직 그래프 조회 중심이며 편성·분리·병합·Staffing 흐름이 없습니다.
- 자동화 화면은 Autonomy Mode 설정이며 자가개선·효과 평가를 보여주지 않습니다.
- 확장 화면은 설치 레코드 목록이며 조직에 추가된 Capability를 설명하지 않습니다.
...
- Growth 도메인, 평가·채택·되돌리기와 CLI/TUI 계약은 존재합니다.
- 현재 데스크톱에서는 이 제품 축을 관찰하거나 통제할 수 없습니다.
- Work 결과가 사용자에게 이해 가능한 개선 후보와 효과 비교로 이어지는 완성 흐름을 제품 Surface에서 검증해야 합니다.
...
추가 화면 코드를 작성하기 전에 대표·홈, 업무, 조직, 결정, 성장, 역량, 설정의 역할과 이동 관계를 목업과 구현 설계로 고정합니다. 기존 Work 관제 화면은 업무 상세로 유지하고 AgentOS 전체 셸로 확장 해석하지 않습니다.

완료 조건:

- 일곱 표면의 사용자 목적과 핵심 행동이 중복되지 않습니다.
