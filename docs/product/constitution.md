...
| 역량 | 조직이 무엇을 할 수 있는가? | Skill, Tool, MCP, Extension, Model Route |
...
| 역량 | Skill·Tool·MCP·Extension과 Provider가 추가하는 실제 능력을 관리 |

하나의 Work를 보는 4영역 관제 화면은 유효한 Work 상세 화면입니다. 그러나 그것이 AgentOS 전체 정보 구조는 아닙니다. `자동화`는 독립적인 제품 축이 아니라 실행 정책과 성장 채택 정책의 설정이며, `확장`은 설치 패키지 목록보다 조직에 추가된 Capability를 먼저 보여줘야 합니다.
...
추가 화면 코드를 작성하기 전에 대표·홈, 업무, 조직, 결정, 성장, 역량의 역할과 이동 관계를 목업으로 고정합니다. 기존 Work 관제 화면은 업무 상세로 유지하고 AgentOS 전체 셸로 확장 해석하지 않습니다.

완료 조건:

- 여섯 표면의 사용자 목적과 핵심 행동이 중복되지 않습니다.
