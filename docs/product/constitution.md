...
| 확인 필요 | 위험 실행, 조직 변경, 권한과 자기수정 채택을 통합 검토 |
...
표면 이름은 2026-07-23에 사용자 승인으로 정했습니다. 표면 집합은 일곱 개 그대로이고 용어만 바뀌었습니다: `결정` → `확인 필요`, `성장` → `개선`, `역량` → `확장`.

- `확인 필요`는 실행이 **멈춰서 사람을 기다리는 것**을 가리킵니다. 알림함이 아닙니다. `awaiting-approval`은 통보가 아니라 정지 상태이며, 이 제품을 "승인 알림함과 자동화 설정 앱"으로 축소하지 않기 위해 이름이 그 사실을 말해야 합니다.
- `개선`은 자가개선의 **근거와 효과**를 소유합니다. 채택할 것이 생겼다는 알림은 `확인 필요`에 뜨지만, 왜 바꾸자는지·반대 근거는 무엇인지·바꾸고 나서 나아졌는지는 이 표면이 소유합니다. 근거를 알림으로 축소하면 4.8의 "근거 기반의 보수적 채택"이 성립하지 않습니다.
...
2026-07-24 기준. 표면 일곱(홈, 업무, 확인 필요, 조직, 개선, 확장, 설정)이 모두 있고 같은 3열 골격을 씁니다.
...
추가 화면 코드를 작성하기 전에 홈, 업무, 조직, 결정, 성장, 역량, 설정의 역할과 이동 관계를 목업과 구현 설계로 고정합니다. 기존 Work 관제 화면은 업무 상세로 유지하고 AgentOS 전체 셸로 확장 해석하지 않습니다.
