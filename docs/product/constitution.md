...
- `수신함`의 미해결 승인 항목은 단순 통보가 아니라 실행이 **멈춰서 사람을 기다리는 상태**입니다. 읽거나 수신함을 닫아도 사라지지 않고 승인·거절·취소·만료 뒤에만 제거합니다. 배지는 읽지 않은 수가 아니라 조직 전체의 미해결 조치 수를 셉니다.
- `개선`은 자가개선의 **근거와 효과**를 소유합니다. 채택할 것이 생겼다는 사실은 `수신함`에 뜨지만, 왜 바꾸자는지·반대 근거는 무엇인지·바꾸고 나서 나아졌는지는 이 표면이 소유합니다. 근거를 수신함 항목으로 축소하면 4.8의 "근거 기반의 보수적 채택"이 성립하지 않습니다.
- `수신함`은 제품 표면이 아니라 어느 표면에서나 여는 전역 유틸리티입니다. 범용 알림 도메인이 생기기 전까지 실제 대기 승인만 투영합니다.
