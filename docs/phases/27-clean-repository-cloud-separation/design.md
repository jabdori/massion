...
> 깨끗한 비공개 `massion` Core 저장소 전환 → 새 저장소에서 Phase 24 검증 재개·완료 → Phase 25 구현 → 개인용 제품 출시 → 실제 관리형 서비스 사업 요구가 생기면 별도 저장소 분리

새 저장소는 legacy source repository의 Git 객체·브랜치·원격을 복사하지 않습니다. 다만 Massion의 구현 시작점부터 현재까지의 의미 있는 제품 개발 커밋 메시지와 변경 계보는 보존합니다.
...
이 시작점부터 구현 기준 종료 commit까지 262개 commit의 제목·본문을 archive-only prohibited marker 목록으로 검사한 결과는 0건입니다. 전환 설계·검증을 기록하는 후속 Phase 27 commit은 같은 허용 경로·금지어 검사 후 추가합니다. Phase 0 이전의 17개 설계·계보 commit은 이력 보존 범위에 넣지 않습니다. 정확한 marker 목록은 새 Core에 넣지 않고 private archive manifest에만 보관합니다.
...
- legacy source lineage 코드·문서·브랜드·조사 기록
...
- 관리형 서비스 사업 코드·URL·database·billing·fleet·SSO·SCIM·SLA 자료
...
- `REQ-REPO-002`: legacy source lineage·관리형 서비스 사업 자료, 원본 Git object, 비밀·개인 경로·산출물은 새 tree와 history에서 0건입니다.
