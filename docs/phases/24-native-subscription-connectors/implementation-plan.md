...
- [x] 현재 source의 기존 profile 재사용 실제 UAT를 사용자 local 환경에서 실행했습니다. 기존 Massion 관리 profile을 재사용해 `connectionDisposition: "reused"`, 같은 account·Connector, 직접 quota 관측을 확인하고, 재로그인 UI가 열리지 않은 결과를 source commit과 함께 `docs/evidence/phase-24/codex-live-profile-reuse-2026-07-15.md`에 고정했습니다. 이번 검증은 현재 local build 기준이며, 별도 release archive gate는 아직 남아 있습니다.
