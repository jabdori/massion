...
- [ ] Slice 1은 원격 배포 SurrealDB 3.2.1 계약(Slice 1A)을 먼저 복원하고, 개인용 로컬 runtime(Slice 1B)은 [개인용 로컬 런타임 설계](./slice-1b-local-surrealdb-3.2.1-design.md)에 따라 TDD로 구현합니다. Slice 1B는 clean reset, native runtime, onboarding, TUI·Web·CLI 공통 기능, personal backup/restore, release verification을 하나의 사용자 제품 흐름으로 닫습니다.
