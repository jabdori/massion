...
이 UAT는 모델 경로가 없는 제한 모드에서 실행됐으므로 Representative는 `blocked_model_unavailable`으로 끝났습니다. Provider 성공, Representative handoff, Context & Strategy memory 사용은 아직 성공으로 기록하지 않습니다.
...
- 실제 Provider 계정으로 Representative 성공, handoff, memory와 quota·fallback을 검증합니다.
