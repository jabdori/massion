...
깨끗한 개인용 설치에서 사용자가 TUI 또는 Web으로 업무를 시작하면, Massion은 Work와 기본 협업방을 만들고 사용자 요청과 Representative의 Context & Strategy handoff를 기록합니다. 두 화면은 정상 실시간 사건 뒤 새 Work·협업방·메시지를 새로고침 없이 같은 데이터로 표시합니다.
...
- [ ] **공통 실제 검증과 기록**
  - 실제 local runtime에서 `run.start → Work → 기본 방 → 요청/handoff → TUI → Web`을 확인합니다.
  - Provider 인증이 필요한 Representative 성공 UAT는 별도 명령과 함께 사용자에게 요청하며, 인증 전에는 성공으로 기록하지 않습니다.
  - 결과만 `implementation-plan.md`와 Phase review에 짧게 기록하고, Cloud·레지스트리·모델 평가실·추가 Provider 기능은 이 조각에 포함하지 않습니다.
