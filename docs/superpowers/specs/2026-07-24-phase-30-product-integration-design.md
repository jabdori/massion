...
| Z.AI | 과거 `glm-5.2` 실계정 Core UAT 2건, `Z_AI_API_KEY` 환경 변수 존재 확인 | 현재 계정·모델·공식 Coding endpoint 사전 검증 후 사용 |
...
1. Z.AI `glm-5.2` 연결 상태를 사전 확인합니다.
...
- GLM 모델이나 공식 Coding endpoint 사전 확인이 실패하면 다른 모델로 조용히 대체하지 않습니다.
