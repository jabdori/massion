...
공개 릴리스는 다음 명령 하나로 설치할 수 있습니다. 설치기는 버전을 고정한 릴리스 매니페스트와 압축 파일의 SHA-256을 확인하고, 사용자 디렉터리에만 파일을 만듭니다.

```sh
curl -fsSL https://raw.githubusercontent.com/jabdori/massion/main/install.sh | bash
```

실행 전에 스크립트를 검토하려면 다음처럼 저장한 뒤 확인합니다.
...
설치기에는 `curl`, `tar`, Node.js 24 이상, Bun 1.3 이상과 `sha256sum` 또는 `shasum`이 필요합니다. 릴리스 압축 파일을 직접 받은 경우에는 아래 수동 설치 절차를 사용합니다.
...
`massion init`을 인자 없이 실행하면 소유자 이메일과 표시 이름을 묻는 온보딩을 시작하고, 기본 개인 서버(`http://127.0.0.1:7331`)를 자동으로 준비합니다. 초기화 전에 인자 없이 `massion`을 실행해도 같은 온보딩을 마친 뒤 TUI를 바로 엽니다. 스크립트나 CI에서는 기존의 명시적 형식도 사용할 수 있습니다.
