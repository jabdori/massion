...
const contributions = ["eventConsumers:github-sync", "surfaceConnectors:github"];
...
for await (const line of lines) {
  ...
  if (frame.protocol !== protocol || !Number.isSafeInteger(frame.sequence))
    throw new Error("GitHub worker RPC frame이 유효하지 않습니다");
  ...
  if (frame.operation === "host.handshake") {
  ...
  } else if (!ready) throw new Error("GitHub worker handshake가 필요합니다");
    ...
    payload = await invokeGitHub(String(frame.payload.contribution), frame.payload.input);
  } else if (frame.operation === "host.stop") {
    operation = "worker.stopped";
    payload = {};
  } else throw new Error("지원하지 않는 GitHub worker operation입니다");
